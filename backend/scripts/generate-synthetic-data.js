...
async function insertSyntheticData(userId = null) {
  try {
    console.log('Generating 1000 synthetic log entries...');
    const syntheticLogs = generateSyntheticLogs(1000);
    
    // Parse the logs
    const parsedLogs = parseLogFile(syntheticLogs);
    ...
    for (let i = 0; i < parsedLogs.length; i++) {
      ...
      processedLogs.push({
        id: result.rows[0].id,
        anomaly_score: logWithAnomaly.anomaly_score
      });
    }

    const anomalies = processedLogs.filter(log => log.anomaly_score > 0).length;
    console.log(`✓ Successfully inserted ${processedLogs.length} log entries`);
...
// Get user ID from command line argument or use null
const userId = process.argv[2] || null;
insertSyntheticData(userId);
